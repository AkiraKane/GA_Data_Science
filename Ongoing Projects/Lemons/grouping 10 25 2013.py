...
#l_train = l_train.join(pd.get_dummies(l_train['Make']))
#l_train = l_train.join(pd.get_dummies(l_train['Model']))
#l_train = l_train.join(pd.get_dummies(l_train['Trim']))
#l_train = l_train.join(pd.get_dummies(l_train['SubModel']))
l_train = l_train.join(pd.get_dummies(l_train['Transmission']))
#l_train = l_train.join(pd.get_dummies(l_train['WheelType']))
#l_train = l_train.join(pd.get_dummies(l_train['Nationality']))
l_train = l_train.join(pd.get_dummies(l_train['Size']))
#l_train = l_train.join(pd.get_dummies(l_train['TopThreeAmericanName']))
l_train = l_train.join(pd.get_dummies(l_train['VNST']))


l_train = l_train.drop(['PurchDate','Color','Auction','Make','Model','Trim','SubModel','Transmission','WheelType','Nationality','Size','TopThreeAmericanName','VNST'],axis=1)
l_train = l_train.dropna()
...
data = l_train[['VehicleAge','VehOdo','IsOnlineSale' ,'MMRAcquisitionAuctionAveragePrice','MMRAcquisitionAuctionCleanPrice','MMRAcquisitionRetailAveragePrice','MMRAcquisitonRetailCleanPrice','MMRCurrentAuctionAveragePrice','MMRCurrentAuctionCleanPrice','MMRCurrentRetailAveragePrice','MMRCurrentRetailCleanPrice']]
