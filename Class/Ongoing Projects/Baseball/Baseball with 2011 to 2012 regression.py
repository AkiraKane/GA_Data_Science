...
lm.fit(X.drop(['playerID'],axis=1), y)

X['predicted'] = lm.predict(X.drop(['playerID'],axis=1))
X['predicted'][X['predicted'] < y_min] = y_min

print 'R-Squared:',lm.score(X.drop(['playerID','predicted'],axis=1), y)
print 'MSE:',metrics.mean_squared_error(lm.predict(X.drop(['playerID','predicted'],axis=1)), y)
print 'MSE:',metrics.mean_squared_error(X.predicted, y)

#Prep Submission File

submission = hitters12.append(pitchers12)
b2012_csv = submission[['playerID', 'salary','predicted']]
...
print 'R-Squared:',lm_H.score(X_H.drop(['playerID'],axis=1), y_H)
# Checking MSE, roughly terrible
print 'MSE:',metrics.mean_squared_error(hitters['predicted'], y_H)
...
# Checking performance, roughly .19
print 'R-Squared:',lm_P.score(X_P.drop(['playerID'],axis=1), y_P)
# Checking MSE, roughly terrible
print 'MSE:',metrics.mean_squared_error(pitchers['predicted'], y_P)
...
((submission['salary_2012'] - submission['predicted'])**2).mean()
